...
// Hanya owner/super_admin yang bisa hapus
if (!isOwner()) {
    redirect('/modules/transactions/', '❌ Akses ditolak!', 'error');
}
...
if ($id) {
    ...
    try {
        ...
        $db->commit();
        redirect('/modules/transactions/', '✅ Transaksi dihapus!');
    } catch (Exception $e) {
        $db->rollBack();
        redirect('/modules/transactions/', '❌ Gagal: ' . $e->getMessage(), 'error');
    }
}
?>