...
function logTransaction(PDO $db, int $transactionId, int $userId, string $action, ?string $oldStatus = null, ?string $newStatus = null, ?string $notes = null): void {
    ...
    $stmt->execute([
        ':tid' => $transactionId, ':uid' => $userId, ':action' => $action,
...
function showFlashMessage(): void {
    if (isset($_SESSION['flash_message'])) {
        ...
        echo '<div style="background:' . $bgColor . ';color:' . $textColor . ';padding:12px 16px;border-radius:8px;margin-bottom:16px;border-left:4px solid ' . $borderColor . ';">' . htmlspecialchars($_SESSION['flash_message']) . '</div>';
